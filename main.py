...
def main():
    ...
    if config["model"].get("use_bootstrap", True):
        ...
        bootstrap_ci_df = pd.DataFrame({"lower": lower_bs, "upper": upper_bs}, index=forecast_index)
        # Evaluate
        ...
        fig.savefig(output_dir / "bootstrap_ci.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
    ...
    if config["model"].get("use_parametric", True):
        ...
        output_dir = ensure_output_dir(config)
        fig.savefig(output_dir / "parametric_ci.png", dpi=300, bbox_inches="tight")
        plt.close(fig)
